...
public class CookieUtil {
    ...
    public void addCookie(HttpServletResponse response, String name, String value, int maxAge) {
        ...
        cookie.setSecure(Boolean.parseBoolean(secure)); // HTTPS 연결에서만 전송할지 설정
        cookie.setAttribute("SameSite", "Strict"); // 쿠키가 이 사이트 이외에서는 요청에 포함되지 않게 설정 CSRF 공격 방지
        response.addCookie(cookie);
    }

    // AccessToken 쿠키 생성
    public  void addResponseAccessTokenCookie(HttpServletResponse response, String accessToken, int maxAge) {
        addCookie(response, "accessToken", accessToken, maxAge);
    }

    // RefreshToken 쿠키 생성
    public void addResponseRefreshTokenCookie(HttpServletResponse response, String refreshToken, int maxAge) {
